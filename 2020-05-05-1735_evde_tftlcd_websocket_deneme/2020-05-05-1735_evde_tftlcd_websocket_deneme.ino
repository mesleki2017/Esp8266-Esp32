...
String basinc;
  ...
  webSocket.broadcastTXT(xekseni);
  webSocket2.broadcastTXT(yekseni);
  webSocket3.broadcastTXT(basinc);
...
void webSocketEvent2(byte num, WStype_t type, uint8_t * payload, size_t length)
{
  if(type == WStype_TEXT)
  {
      if (payload[0] == '0')
...
void webSocketEvent3(byte num, WStype_t type, uint8_t * payload, size_t length)
{
  if(type == WStype_TEXT)
  {
      if (payload[0] == '0')
