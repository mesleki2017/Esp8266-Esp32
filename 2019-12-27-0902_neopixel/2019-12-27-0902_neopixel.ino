...
javasciptenRenkText4 = server.arg("RcolorSag"); //Refer  xhttp.open("GET", "setLED?xxx="+today.getTime(), true)
 ...
 Serial.print("alinanRcolorSag=> ");Serial.println(alinanRcolorSag);
...
void espyeParlaAl() {
javasciptenRenkText7 = server.arg("ParlaEsp"); //Refer  xhttp.open("GET", "setLED?xxx="+today.getTime(), true)
Serial.print("ParlaEsp=> ");Serial.println(javasciptenRenkText7);

String str1="parlaEspAlindi";
server.send(200, "text/plane",str1 ); 
for(int i=0; i<1000; i++) {
pixels.setPixelColor(random(0,24), pixels.Color(random(0,255),random(0,255),random(0,255)));
pixels.show();
}
}
...
void setup(void){
  ...
  server.on("/parlaAl", espyeParlaAl);
