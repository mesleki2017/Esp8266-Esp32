...
int ApiHavaJsondanZamanBilgisi(int i){//yeni bilgi int tipi fonksiyon da return asagidaki gibi olabiliyor. int tipi fonksiyonlari ilk defa bu
  //programda kullandim. 
  String zamanTxt;
  time_t apiHavaZaman=0;
  serializeJson(doc["list"][i]["dt"], zamanTxt);//ApiHavaJsondanZamanBilgisi json dan zamanı saniye formatinda aliyor
  apiHavaZaman=zamanTxt.toInt();
  return LcdYeZamanBilgisiYaz(apiHavaZaman);
 }
 ////////////////////////////////////////////////////////////////////////////////////////////////
 int LcdYeZamanBilgisiYaz(time_t zaman){
  String text1="";
  tft.setTextColor(0x6043,0x6d56);
  text1=String(day(zaman))+ String("-")+ay[month(zaman)]+ String(" ");
  if (hour(zaman) < 10)text1=text1+String("0");
  text1=text1+String(hour(zaman))+ String(":");
